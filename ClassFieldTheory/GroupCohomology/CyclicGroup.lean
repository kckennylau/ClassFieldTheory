import CFT.Cohomology.LeftRegular
...
open Classical Finsupp IsCyclic
open Rep hiding of
...
notation "diff₁[" R "," G "]" => diff₁ (R := R) (G := G)

@[simp]
lemma diff₁_of_one : diff₁[R,G].hom (of 1) = of 1 - of gen :=
  leftRegularHom_apply (of 1 - of gen)

lemma diff₁_of (g : G) : diff₁[R,G].hom (of g) = of g - of (g * gen) :=
by
  simp only [of_eq_ρReg_of_one g, hom_comm_apply, diff₁_of_one, map_sub, sub_right_inj]
  rw [ρReg_apply_of]

@[simp] theorem ε_comp_diff₁ : diff₁[R,G] ≫ ε = 0 :=
    Hom.ext _ _ (by simp)

/-
For any `g : G`, the element `of g - of 1` of the left regular representation is in the image of `diff₁`.
This is a step on the way to proving that the image of `diff₁` is equal to the kernel of `ε`.
-/
lemma of_sub_of_one_mem_range_diff₁ (g : G) : of g - of 1 ∈ LinearMap.range diff₁[R,G].hom.hom := by
  ...
  | ofNat n =>
    ...
    | succ n ih =>
      ...
      use ρReg gen w - of 1
      simp only [apply_eq_hom, map_sub, diff₁_of_one]
      rw [hom_comm_apply, hw, ←ρReg, map_sub, ρReg_apply_of, ρReg_apply_of]
      simp only [mul_one, sub_sub_sub_cancel_right, sub_left_inj]
      norm_cast
      rw [pow_succ']
  | negSucc n =>
    ...
    | succ n ih =>
      ...
      rw [map_add, diff₁_of, hom_comm_apply, ←ρReg, hw, map_sub,
...
-/
theorem exact_ε_diff₁ : LinearMap.ker ε.hom.hom = LinearMap.range diff₁[R,G].hom.hom :=
by
  apply eq_of_le_of_le
  · intro v hv
    rw [LinearMap.mem_ker, ε_eq_sum_coeff] at hv
    let pre (g : G) : leftRegular R G :=
      (of_sub_of_one_mem_range_diff₁ g).choose
    have pre_spec (g : G) : diff₁.hom.hom (pre g) = of g - of 1 :=
      (of_sub_of_one_mem_range_diff₁ g).choose_spec
    let S := (coeff v).support
    use ∑ g ∈ S, coeff v g • pre g
    rw [map_sum]
    simp only [LinearMapClass.map_smul, pre_spec, smul_sub]
    ext x
    rw [map_sum]
    simp only [map_sub, map_smul, Finset.sum_sub_distrib, coe_sub, coe_finset_sum, coe_smul,
      Pi.sub_apply, Finset.sum_apply, Pi.smul_apply, coeff_of, smul_eq_mul, mul_ite, mul_one,
      mul_zero, Finset.sum_ite_eq', Finset.sum_ite_irrel, Finset.sum_const_zero]
    by_cases hx : x ∈ S
    · rw [if_pos hx]
      split_ifs
      · rw [hv, sub_zero]
      · rw [sub_zero]
    · symm
      rwa [if_neg hx, zero_sub, hv, ite_self, neg_zero, ←not_mem_support_iff]
  · rw [LinearMap.range_le_ker_iff]
    ext
    change (diff₁ ≫ ε) _ = 0
    simp only [ε_comp_diff₁, Action.zero_hom, ModuleCat.hom_zero, AddHom.toFun_eq_coe,
      LinearMap.coe_toAddHom, LinearMap.zero_apply]


theorem exact_ε_diff₁' (v : leftRegular R G) (hv : ε.hom.hom v = 0) :
  ∃ w : leftRegular R G, v = diff₁.hom w :=
by
  let pre (g : G) : leftRegular R G :=
    (of_sub_of_one_mem_range_diff₁ g).choose
  have pre_spec (g : G) : diff₁ (pre g) = of g - of 1 :=
    (of_sub_of_one_mem_range_diff₁ g).choose_spec
  let S := (coeff v).support
  use ∑ g ∈ S, coeff v g • pre g
...
lemma coeff_diff₁_gen_mul (v : leftRegular R G) (g : G) :
    coeff (diff₁ v) (gen * g) = coeff v (gen * g) - coeff v g :=
by
