...
\begin{example} \label{eg:Hn unit}
...
\begin{example} \label{eg:homology unit}
	...
	$H_n(1,M)$.
	This follows in a similar way to \ref{eg:Hn unit}
\end{example}
...
\[
	\coind_\phi(M) ; \{ f : G \to M| \forall s \in S, x \in G, f(\phi(s)x) = s \bullet f(x)\}.
\]
This can be thought of as the $(G \to M)^S$, where the action of $S$ is
...
\begin{corollary} \label{eq:Tate -1 aug}
	\uses{lem:homology 0 aug,
	...
	\[
		H^{-1}_{\Tate}(G,\aug(R,G)) \cong H_{0}(G, \aug(R,G)) \cong G^{\ab} \otimes R.
	\]
\end{corollary}

\begin{proof}
	The first isomorphism follows because the map $N_G : \aug(R,G) \to \aug(R,G)$ is zero.
	The second isomorphism is \ref{lem:homology 0 aug}.
\end{proof}
...
\begin{proof}
	...
	This follows because $\rest(\sigma) \in H^n(1,M) \cong 0$ by \ref{eg:Hn unit}.
\end{proof}
...
This is given by
\[
	\map_2(f) (x) = f(x) - f(\gen \cdot x), \qquad i.e.\;
	\map_2(\single(x,m)) = \single(x,m) - \single(\gen^{-1} \cdot x, m).
\]
...
We next a formula for the reciprocity isomorphism in terms of
...
\begin{proof}
	...
	\[
		...
		\end{matrix}
	\]
	Choose an element $g \in G$. Recall that the element $gG' \otimes 1 \in G^{\ab} \otimes R$
...
\begin{proof}
	...
	is a fundamental class follows from \ref{lem:restriction fundamental class generates}.
	...
	\]
	Using the cocyclce relation we have
	\[
