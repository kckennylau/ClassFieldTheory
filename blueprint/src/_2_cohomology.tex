...
following result, which is a current PR (\# 25937, \# 25996):
...
\begin{proof}
	...
	\]
	The right hand side is $N_{G/S} \theta_S(s)$.
\end{proof}
...
\begin{corollary}
	...
	Let $(R,G,M)$ be a finite class formation and let $S$ be a subgroup of $G$.
	Then the image of $S$ under the reciprocity map $G^{\ab} \to M^G / N_GM$ is
	$N_{G/S}(M^S) / N_GM$.
\end{corollary}

\begin{proof}
	This follows from the commutative diagram im \ref{lem:subgroup compatibility}.
\end{proof}

\begin{corollary} \label{cor:subgroup le iff norm module le}
	Let $(R,G,M)$ be a finite class formation
...
\begin{corollary} \label{cor:norm limitiation}
	Let $S$ be a subgroup of $G$. Then
	\[
		N_{G/S} (M^S) = N_{G/SG'} (M^{SG'}).
	\]
\end{corollary}

\begin{corollary} \label{cor:abelian subgroup le iff norm module le}
	Let $G$ be abelian and let $S_1$ and $S_2$ be subgroups of $G$.
